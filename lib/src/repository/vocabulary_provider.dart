  ...
  Future<List<PhrasalVerb>> _getPhrasalVerbs() async {
    ...
    return rows
        .map((row) => PhrasalVerb(
              ...
              progressiveEs: row[6],
              presentIEs: row[7],
              presentSingularYouEs: row[8],
              presentHeEs: row[9],
              presentWeEs: row[10],
              presentTheyEs: row[11],
              pastIEs: row[12],
              pastSingularYouEs: row[13],
              pastHeEs: row[14],
              pastWeEs: row[15],
              pastTheyEs: row[16],
              isTransitive: row[17] == 1,
              isDitransitive: row[18] == 1,
              canBeLinkingVerb: row[19] == 1,
              help: row[20],
              particle: row[21],
              isSeparable: row[22] == 1,
            ))
        .toList();
  }
